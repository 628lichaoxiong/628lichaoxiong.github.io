            ...
            <div class="tiaojian_wrap tr clear" style="float: right; height:30px;">
    			<div class="uboxstyle ubox_sele card_select" style="float:left; ">
    				<select id="optype" name="optype">
    				  <option value="1" <c:if test="${params.type!=null}">selected</c:if>>类型</option>
    				</select>
    			</div>
				
				<div class="uboxstyle ubox_sele card_select" style="float:left; ">
    				<select id="optype1" name="optype1">
    				  <option value="1" <c:if test="${params.type!=null}">selected</c:if>>卡片</option>
    				</select>
    			</div>
		...
		<div class="table-list">
			 ...
			 <nav> <!-- 分页开始 -->
					...
					<pageNation:PageNation currPage="${list.data.result.size+1}"
							...
							linkBaseUrl="getSysLogList?search=${params.search}&${params.url}&url=${params.url}">
...
function searchMeeting(){
    ...
    if(optype==1){
        var url="type="+search_key;
        window.location.href="${rc.contextPath}/sysLog/getSysLogList?type="+search_key+"&search="+search_key+"&url="+url;
    }
