		...
		<div class="row">
			...
			<div class="col-xs-9">
				...
				<form role="form" method="post"
					action="${cbasePath}user/updateUser?logoURL=${resuserDetail.data.result.logoURL}&robot=${resuserDetail.data.result.robot}&userid=${resuserDetail.data.result.id}&categoryids2=${categoryids}"
					...
					<div class="form-group">
						...
						cols="20" value="${resuserDetail.data.result.intro }">${resuserDetail.data.result.intro }</textarea>	
					</div>
					
					<div class="form-group">
					<label for="exampleInputEmail1">大咖简介</label>
					<textarea name="Vinfo" class="form-control" rows="3"
						cols="20" value="${resuserDetail.data.result.vinfo }">${resuserDetail.data.result.vinfo }</textarea>
					</div>
	
					<div class="form-group">
						<label for="exampleInputEmail1">我的职业</label> <input type=""
							name="station" class="form-control" id="exampleInputEmail1"
							placeholder="" value="${resuserDetail.data.result.station }">
					</div>

					<div class="form-group">

						<br>原二级分类：${categorynames}<br> <label
							for="exampleInputEmail1">二级分类</label>
					 ...
					 --%>
						<c:forEach items="${currCategory.data.result.childCategory}"
							var="cate">
							<label for="exampleInputEmail1"> <input type="checkbox"
								name="categoryids" value="${cate.id}" /> ${cate.categoryName}
							</label>
						</c:forEach>
					</div>
