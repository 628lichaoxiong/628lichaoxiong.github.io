				...
				<c:if test="${resuserList.status == '200'}">
					...
					<ul class="pagination">
						<pageNation:PageNation currPage="${resuserList.data.result.size+1}"
							totalPages="${resuserList.data.result.totalpage}" perPageRows="10"
							totalRows="${resuserList.data.result.total}"
							linkBaseUrl="${cbasePath}user/userList?id=">
					...
					<c:forEach items="${resuserList.data.result.datas}" varStatus="key" var="Recourse">
						<div class="row" style="padding: 20px;">
							...
							<div class="col-xs-1">
								<div class="row">
									<img class="col-xs-12 thumbnail" src="${Recourse.logoimage}"
							...
							<div class="col-xs-10">
								<h4 style="margin-left:12px;">
									<a href="${cbasePath}user/getuserinfo?userid=${Recourse.userid}">
										用户昵称：${Recourse.username}<!--  </a> --><small><small
